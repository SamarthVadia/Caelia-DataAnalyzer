function condensateFraction = peakVisibilityV6_2(u, eachplot)
...
%figure(10), plot(peakfit0); hold on; plot(projection); plot(lowpass-2); hold off;
...
smoothed = real(ifft(ifftshift( fftshift(fft(resid1)).*(exp((-linspace(-r/2,r/2,r).^2)/((r*20)/filter)))')));
...
%figure(11), plot(projection); hold on; plot(peakfit2); hold off
...
%figure(20),plot(thermal+SattPeaks+CenterPeak+offset), hold on, plot(projection), hold off
%figure(21), plot(thermal+offset), hold on , plot(projection),hold off
%figure(22), plot(thermal+SattPeaks+CenterPeak), hold on, plot(projection),plot(SattPeaks),plot(CenterPeak), hold off

baseValue = min(thermal+offset);
%condensateFraction = (Ca*Cc + (S1a + S2a)*Sc)/((Ca*Cc + (S1a + S2a)*Sc)+Ta*Tc+offset*r);
condensateFraction = sum(CenterPeak+SattPeaks)/sum(CenterPeak+SattPeaks+thermal+offset-baseValue);
