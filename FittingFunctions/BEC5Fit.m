...
%     size(zData)
    ...
    ft = fittype( 'nTotal*(1-cf)/(2*pi*(rx*sx)*(ry*sy)*1.2021)*gbec(2,exp(-((x-x0)^2/(rx*sx)^2+(y-y0)^2/(ry*sy)^2)/2),30)+nTotal*cf*5/(2*pi*rx*ry)*max((1-(x-x0)^2/rx^2-(y-y0)^2/ry^2),0)^(3/2)', 'independent', {'x', 'y'}, 'dependent', 'z', 'coefficients', {'cf','nTotal','rx','ry','sx','sy','x0','y0',});
