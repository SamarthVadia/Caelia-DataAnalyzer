...
%}
    ...
    % Find position of SF peak maxima
...
%     size(zData)
    ...
    ft = fittype( '(1-cf)*nTotal/(2*pi*sx*sy)/1.202*gbec(2,exp(-((x-x0)^2/sx^2+(y-y0)^2/sy^2)/2),3)+nTotal*(cf-cfs)*5/(2*pi*rx*ry)*max((1-(x-x0)^2/rx^2-(y-y0)^2/ry^2),0)^(3/2)+nTotal*cfs*5/(4*pi*rx*ry)*max((1-(x-(x0-x1))^2/rx^2-(y-(y0-y1))^2/ry^2),0)^(3/2)+nTotal*cfs*5/(4*pi*rx*ry)*max((1-(x-(x0+x1))^2/rx^2-(y-(y0+y1))^2/ry^2),0)^(3/2)', 'independent', {'x', 'y'}, 'dependent', 'z', 'coefficients', {'cf','nTotal','sx','sy','x0','y0','rx','ry','cfs','x1','y1'});
    
    opts = fitoptions( 'Method', 'NonlinearLeastSquares' );

    opts.Lower =        [0     0.7*nTotal   xThFWHM/10    yThFWHM/10    xpeak-30    ypeak-30   xCondFWHM/10     yCondFWHM/10    0     0              0];
    opts.StartPoint =   [0.8    nTotal      xThFWHM       yThFWHM       xpeak       ypeak      xCondFWHM        yCondFWHM       0.1   1              y1guess];
    opts.Upper =        [1      1.3*nTotal  Inf           Inf           xpeak+30    ypeak+30   Inf              Inf             0.5   xThFWHM        y1guess*10];
